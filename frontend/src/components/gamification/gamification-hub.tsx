            ...
            <CardContent>
                <div className="flex justify-between items-end mb-2">
                    <div>
                        ...
                        <span className="text-xs text-muted-foreground ml-1">{t('gamification.xpLabel')}</span>
                    </div>
                    <span className="text-xs text-muted-foreground">{t('gamification.levelLabel').replace('{level}', String(level))}</span>
                </div>

                <Progress value={progress} className="h-2 bg-orange-500/10" indicatorClassName="bg-gradient-to-r from-orange-400 to-amber-500" />
                <p className="text-xs text-muted-foreground mt-2 text-right">{t('gamification.nextLevelProgress').replace('{remaining}', String(nextLevel - points)).replace('{level}', String(level + 1))}</p>
                ...
                <div className="mt-4 pt-4 border-t border-orange-500/10 grid grid-cols-4 gap-2">
                    {data?.badges && data.badges.length > 0 ? (
                        ...
                        <div className="col-span-4 text-center py-2 text-xs text-muted-foreground">
                            {t('gamification.completeMissions')}
                        </div>
                    )}
